...
class MassExerciseRefactor:
    def __init__(self, project_root: str):
        ...
        self.replacement_patterns = [
            ...
            # 2. CSVExercise.objects -> CSVExercise.objects
            {
                'pattern': r'\bExercise\.objects\b',
                'replacement': 'CSVExercise.objects',
                'description': 'CSVExercise.objects replacement'
            },
            
            # 3. CSVExercise.DoesNotExist -> CSVExercise.DoesNotExist
            {
                'pattern': r'\bExercise\.DoesNotExist\b',
                'replacement': 'CSVExercise.DoesNotExist',
                'description': 'CSVExercise.DoesNotExist replacement'
            },
            
            # 4. CSVExercise( -> CSVExercise(
            {
                'pattern': r'\bExercise\(',
            ...
            # 5. spec=CSVExercise -> spec=CSVExercise (для Mock)
            {
                'pattern': r'spec=CSVExercise\b',
                'replacement': 'spec=CSVExercise',
                'description': 'Mock spec replacement'
            },
            
            # 6. # @admin.register(Exercise) - REMOVED in Phase 5.6 -> удалить (уже удалено из models)
            {
                'pattern': r'@admin\.register\(Exercise\)',
                'replacement': '# # @admin.register(Exercise) - REMOVED in Phase 5.6 - REMOVED in Phase 5.6',
        ...
        self.complex_patterns = [
            ...
            'exercise.name_ru -> exercise.name_ru', 
    ...
    def fix_csvexercise_fields(self, content: str) -> Dict[str, any]:
        ...
        # 1. exercise.id -> exercise.id (CSVExercise использует id вместо slug)
        pattern = r'(\w+)\.slug'
        ...
        if count > 0:
            modified_content = new_content
            changes.append(f"exercise.id -> exercise.id: {count} замен")
        
        # 2. exercise.name_ru -> exercise.name_ru (предпочтительно русское имя)
        ...
        if count > 0:
            modified_content = new_content
            changes.append(f"exercise.name_ru -> exercise.name_ru: {count} замен")
        
        # 3. exercise.level -> exercise.level
        ...
        if count > 0:
            modified_content = new_content
            changes.append(f"exercise.level -> exercise.level: {count} замен")
        
        # 4. Исправление создания Exercise объектов
        # CSVExercise.objects.create(id=..., name_ru=..., name_en=..., level=..., muscle_group="full_body", exercise_type="strength", is_active=True) ->
        # CSVExercise.objects.create(id=..., name_ru=..., level=..., muscle_group="full_body", exercise_type="strength", is_active=True)
        ...
        def fix_create_params(match):
            ...
            return f'CSVExercise.objects.create({params}, muscle_group="full_body", exercise_type="strength", is_active=True)'
        
        new_content = re.sub(create_pattern, fix_create_params, modified_content)
        if new_content != modified_content:
            modified_content = new_content
            changes.append("Исправлены параметры CSVExercise.objects.create() для добавления обязательных полей")
    ...
    def print_statistics(self):
        ...
        print(f"   CSVExercise.objects исправлено: {self.stats['exercise_objects_fixed']}")
        ...
        for file_path in python_files:
            try:
                ...
                problematic_patterns = [
                    ...
                    r'from.*CSVExercise(?!Validation)',  # Exercise но не ExerciseValidation
                    r'import.*CSVExercise(?!Validation)'
                ]
